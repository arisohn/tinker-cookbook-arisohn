"""On-policy distillation using `tinther` in place of `tinker`.

This script is effectively ``tinker_cookbook/distillation/train_on_policy.py``
...
Launch (multi-GPU)::
    ...
    accelerate launch --num_processes=$NGPUS \
        ...
        tinker_cookbook/distillation/train_on_policy_tinther.py <chz args...>

Single GPU::

    TINTHER_BACKEND=ddp python tinker_cookbook/distillation/train_on_policy_tinther.py <chz args...>
"""
...
# import runs. Importing via the package path (``from tinker_cookbook...``)
# would trigger ``tinker_cookbook/distillation/__init__.py``, which eagerly
# imports the three cookbook training modules — each of which does
# ``import tinker``. That would fail before we got the chance to install
# the alias. Loading by file path avoids running any package __init__.
...
from tinker_cookbook.distillation.train_on_policy import Config, main  # noqa: E402
